...
func tick(actor: Node, blackboard: Blackboard) -> int:
	...
	var collision_point := Utils.calculate_projectile_and_target_collision_point(target.global_position, target.ship.linear_velocity, npc.ship.global_position, npc.ship.linear_velocity, npc.ship.weapon_set.get_projectile_speed())
